...
> **Pilot exception (session 026 — dual-truth window).** The manual-location story and its
> children are tracked in **mtt** (`.mtt/`, story `s1`; see the "Working under mtt" section below).
> **Everything else stays in beads** until the go/rollback gate — do not re-file the rest of the
> backlog into mtt before GO. On GO this note is removed and the beads rules below are replaced by mtt.
