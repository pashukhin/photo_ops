...
Invoked by `scripts/test-guard` as:
    python -m scripts.testguard.cli <base_ref>
...
def _git(*args: str) -> str:
    ...
    Raises RuntimeError if git exits non-zero — a broken invocation must not
    silently pass the gate by returning an empty diff.
    """
...
def main() -> int:
    ...
    try:
        ...
        print(f"test-guard: {exc}", file=sys.stderr)
        return 1
