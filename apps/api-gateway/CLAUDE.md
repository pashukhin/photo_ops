...
- Logging / correlation: structured JSON via `nestjs-pino` (`makeLoggerOptions` from `@photoops/observability`); OTel propagation started in `src/tracing.ts`; HTTP requests auto-logged; mapped errors logged by `HttpErrorFilter` (4xx warn / 5xx error).
