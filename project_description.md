...
## 13. Продуктовые фичи, выявленные при планировании (backlog)

Список ведётся, чтобы не потерять продуктовые идеи, всплывшие при планировании
сессий, но ещё не входящие в зафиксированный MVP-объём. Это не обязательства, а
осознанно отложенный backlog; каждый пункт уезжает в свою стадию роадмапа.

Выявлено при планировании сессии 008 (media processing):

* **Детальный pay-as-you-go биллинг с расшифровкой «за что и сколько».**
  Расширяет §3.10: не только демонстрация метрик, а детализированный счёт по
  потреблённым ресурсам, корректный к оспариванию пользователем (charge-once,
  без двойных списаний). Архитектурный задел уже заложен (учётная единица
  прогона обработки).
* **Намеренная реобработка фото как отдельное оплачиваемое действие.**
  Пользователь может перезапустить обработку (новые правила вариантов, новый
  формат и т.п.); это отдельный тип задачи с отдельной тарификацией, в отличие
  от системного ретрая. Шов в архитектуре подготовлен.
* **Расширение поддерживаемых форматов входа, включая RAW с камер.**
  Расширяет §3.2 (сейчас JPEG-минимум). Оригинал сохраняется **без изменений и
  без транскодирования** (raw→jpeg — потеря качества); thumbnail/preview —
  производные. Пайплайн обработки спроектирован format-agnostic по входу.
* **Анализ фотобанка по параметрам съёмки.** Пользовательский анализ/фильтрация
  по ISO, выдержке, диафрагме, объективу, фокусному расстоянию. Расширяет §3.3
  (там EXIF только извлекается). Параметры уже сохраняются в raw-метаданных, так
  что фича не потребует переобработки — только промоут полей.
* **Рендеры под конкретный приёмник публикации.** Варианты изображений могут
  задаваться целевой платформой (Telegram, Instagram, LiveJournal и т.п.) со
  своими правилами (кроп, фильтры, размеры). Расширяет §3.9 и модель
  `PhotoVariant`. Сейчас не делается; измерение `target` в модели вариантов
  названо как будущий шов.
* **Разрешение часового пояса даты съёмки + пользовательское уточнение.**
  Приведение `taken_at` к UTC, когда данных достаточно (EXIF offset или GPS-time);
  иначе сохраняем локальное время и даём пользователю возможность задать/исправить
  TZ позже. Предполагаемую зону можно восстановить по геолокации (обратный
  геокодинг) и предложить пользователю на подтверждение. Связано с §3.4.
* **Богатый UI фотогалереи.** Живая таблица фото с сортировкой, фильтрами и
  пагинацией; клик по строке открывает модалку с детальной информацией и
  превью. Детализирует §3.2/§5; запланировано как сессия 009.

Выявлено при планировании сессий 012–013 (учёт потребления и кластеризация):

* **Иерархическая кластеризация (дерево вместо плоского списка).** Результат —
  дерево кластеров (узлы — кластеры, листья — ссылки на фото); публикацию можно
  создать из кластера **любого уровня**. Расширяет §3.5 и модель `PhotoCluster`/
  `PhotoClusterItem` (§6) до иммутабельных снапшотов-деревьев. Детали дизайна — в
  заготовке `sessions/013_photo_clustering.md`; формализуется ADR в сессии 013.
* **Детерминированная кластеризация в метрическом пространстве-времени.** Уточняет
  §3.5 «без ML»: используются **детерминированные unsupervised-алгоритмы без
  обученных моделей** (agglomerative сейчас; HDBSCAN/OPTICS/BIRCH — сменные, выбор
  пользователем, дефолт позже). Расстояние — евклидова масштабированная метрика
  пространства-времени; «место» входит координатами (reverse-geocoding не требуется
  для алгоритма — отложен, `photo_ops-3iy`).
* **Причинностный (light-cone) детектор аномалий.** Пометка «физически невозможных»
  пар (подброшенное фото / параллельные съёмки) поверх метрики. Будущий шов поверх §3.5.
* **Иммутабельные результаты + мягкое восстановимое удаление + carry-over.** Перекластеризация =
  новый сосуществующий результат; перенос производных объектов на идентичные кластеры —
  опциональный шов. Уточняет «read-only кластер» из §3.5.
* **Учёт потребления как cross-cutting Go-плоскость.** Измерение расхода — в рантайме
  операции (сырые провайдеро-независимые единицы: интеграл памяти по времени,
  cpu/wall-секунды); агрегация/леджер/оценка стоимости — отдельный дешёвый
  конкурентный сервис на Go (`usage-service`). Реализует §3.10 без vendor-lock;
  заготовка `sessions/012_usage_accounting_go_service.md`.
* **Заметки как возможный отдельный `notes-service`.** Универсальные заметки (§3.6)
  — отдельный концерн неизвестной пока необходимости; не вшиваются в доменные сервисы,
  ссылаются на сущности через `entity_type`+`entity_id`.
