...
Процессное решение: эта сессия помечена как «Session I», а не очередным продуктовым номером, потому что она консолидирует фундамент, а не добавляет продуктовый слой. Соответствующий отчёт идёт под `sessions/00i_fortification_review.md`, вне сквозной нумерации 001–003.
